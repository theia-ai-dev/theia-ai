...
def model(model_name):
    return theia.models.get(model_name)
...
def training(model_name, version):
    training_input = {
        # "train_data": "https://storage.googleapis.com/theia-weights/training-deadlock/1k_samples.jsonl",
        "train_data": "https://pub-3054bb37389944ca9c8e5ada8572840e.r2.dev/samsum.jsonl",
    }
    return theia.trainings.create(
        version=model_name + ":" + version.id,
        input=training_input,
        destination="theia-internal/training-scratch",
    )
...
def test_post_training_predictions(trained_model_and_version, prediction_tests):
    ...
    model = theia.models.get(trained_model)
    version = model.versions.get(trained_version)
    predictions = [
        theia.predictions.create(version=version, input=val)
